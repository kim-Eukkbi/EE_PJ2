...
public class JudgementManager : MonoBehaviour
{
    ...
    public float perfectSize = 0.50f;
    public float greatSize = 0.105f;
    public float missSize = 2;
    ...
    private void Update()
    {
        ...
        else if (NoteJudgementCheck(InputManager.instance.IsSpaceDown,
                InputManager.instance.IsSpaceDown,
                InputManager.instance.IsSpaceDown,
                whatIsLongNote))
        {
    ...
    {
        Collider2D[] judgeCols = Physics2D.OverlapBoxAll(GameManager.instance.judgeLine.transform.position + (GameManager.instance.circle.transform.up * (missSize * (NoteManager.instance.noteSpeed / 2.35f))),
                    missJudgementSize,
        ...
        if (perfect && judgementTime < perfectSize)
        {
            Perfect(note);
        ...
        else if (miss && judgementTime < missSize)
        {
            Miss(note);
    ...
    private void OnDrawGizmos()
    {
        ...
        DrawBox(m, m2, missPos + (Vector2)(GameManager.instance.circle.transform.up * (missSize * (NoteManager.instance.noteSpeed / 2.35f))), missJudgementSize, Color.red);
